...
# load data
medicaid_expansion <- read_csv('/Users/taylorgaldi/Computational-Social-Science-Training-Program/Projects/Project 7/data/medicaid_expansion.csv')
...
# Fit a linear model to calculate the slope of the uninsured rate trend pre-2014
state_slopes <- compare_states %>%
  group_by(State, Adopted) %>%
  do({
    lm_model <- lm(uninsured_rate ~ year, data = .)  # Fit a linear model
    slope_value <- coef(lm_model)["year"]  # Extract the slope 
    # Create a data frame with the slope and all other columns
    data.frame(., slope = slope_value)  # Make sure the original columns are in the dataframe too
  })

# Ensure 'Adopted' is a factor for coloring
#state_slopes_sorted_desc$Adopted <- factor(state_slopes_sorted_desc$Adopted, levels = c("Never", "Expanded"))

# Plot the slopes with ggplot2
