...
knitr::opts_chunk$set(
  ...
  message = FALSE,  # (optional) suppress messages too
  echo = TRUE       # still show your code
)
...
#I'm taking out ranger because it was causing too many issues :(
sl_libs <- c('SL.mean', 'SL.glm', 'SL.earth', 'SL.step')

# Safe wrapper to run LTMLE without breaking the knit
safe_ltmle <- function(...) {
  tryCatch({
    ltmle(...)
  }, error = function(e) {
    warning(paste("LTMLE failed:", e$message))
    return(NULL)
  })
}

# Safe extraction of ATE from a fitted ltmle object
safe_extract_ate <- function(fit, model_name = "Unknown") {
  if (is.null(fit)) {
    warning(paste("ATE extraction skipped for", model_name, "- model is NULL."))
    return(data.frame(
      Model = model_name,
      ATE = NA,
      CI_Lower = NA,
  ...
  tryCatch({
    est <- summary(fit)$effect.measures$ATE
    data.frame(
      Model = model_name,
      ATE = round(est$estimate, 4),
      CI_Lower = round(est$CI[1], 4),
      CI_Upper = round(est$CI[2], 4),
      P_value = round(est$pvalue, 4)
    )
  }, error = function(e) {
    warning(paste("ATE extraction failed for", model_name, ":", e$message))
    data.frame(
      Model = model_name,
      ATE = NA,
      CI_Lower = NA,
...
ltmle_full <- safe_ltmle(
  ...
  estimate.time = TRUE,
  gbounds = c(0.01, 0.99),   
  survivalOutcome = FALSE,   #Again, not really sure what anything below this in the ltmle is doing, but it kept breaking
...
# Extract results
naive_results <- safe_extract_ate(ltmle_naive, "Naive LTMLE")
full_results  <- safe_extract_ate(ltmle_full, "Full LTMLE")
